    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (args.Length >= 2)
            {
                ...
                else if (option == "dumprecentlogons")
                {

                    var timeStamp = DateTime.Now.ToFileTime();
                    var threeMonthTime = timeStamp - 26488540000000; //(1 month currently) //3 months: 79488540000000; // current time subtracted by 3 months, to get the timestamp of 3 months ago.

                    string query = "";
                    string properties = "name,givenname,displayname,samaccountname,adspath,distinguishedname,memberof,ou,mail,proxyaddresses,lastlogon,pwdlastset,mobile,streetaddress,co,title,department,description,comment,badpwdcount,objectcategory,userpassword,scriptpath,managedby,managedobjects,serviceprincipalname";
                    try
                    {
                        query = "(&(objectCategory=user)(lastLogon>=" + threeMonthTime + "))";
                        Console.WriteLine("query is " + query);
                        LdapQuery(domain, query, properties);
                    }
                    catch (Exception e)
                    {
                        Console.WriteLine("ERROR: DumpRecentLogons catched an unexpected exception");
                        ShowDebug(e, verboseDebug);
                    }
                }
